...
import com.example.pickfresh.seller.addOns.ViewMany
import com.example.pickfresh.seller.addOns.ViewRentals
...
class SellerMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bind.schemePoint.setOnClickListener {
            startActivity(
                Intent(
                    this,
                    Schemes::class.java
        ...
        val back = BackLogic()
        bind.recommendCrop.text = back.pickTheMonth()

        bind.rentalMachineBTN.setOnClickListener {
            startActivity(
                Intent(
                    this, ViewRentals::class.java
                )
            )
        }
        bind.fertilizerPoint.setOnClickListener {
            startActivity(Intent(applicationContext, ViewMany::class.java))
        }
        swith = bind.switche
        status = bind.status
        ...
        onewordchange.observer().observe(this) {
            if (it.size == string.size) {
                ...
                bind.rentalMachine.text = it[6]
                bind.fertilizerText.text = it[7]
                bind.schemes.text = it[8]
                bind.recommendCrop.text = it[9]
            }
        }
        ...
        string.add(bind.rentalMachine.text.toString())
        string.add(bind.fertilizerText.text.toString())
        string.add(bind.schemes.text.toString())
        string.add(bind.recommendCrop.text.toString())
                ...
                fused.lastLocation.addOnSuccessListener {
                    it?.let { the ->
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                            Geocoder(this@SellerMainActivity).getFromLocation(
                                it.latitude,
                                it.longitude,
                                1
                            ) {
        ...
        CoroutineScope(IO).launch {
            Retrofit.instance.getuser(
                ...
                override fun onFailure(call: Call<LoginResponse>, t: Throwable) {
                    dialog.dismiss()
                    Toast.makeText(this@SellerMainActivity, "${t.message}", Toast.LENGTH_SHORT)
                        .show()
                }
            })
        }
                    ...
                    response.body().apply {
                        if (this != null) {
                            if (error) {
                                ...
                                val k = getSharedPreferences("user", MODE_PRIVATE).getString(
                                    "language",
                                    ""
                                )
                                if (k != "English") {
    ...
    private fun data(string: ArrayList<String>) {
        ...
        bind.rentalMachine.text = string[6]
        bind.fertilizerText.text = string[7]
        bind.schemes.text = string[8]
        bind.recommendCrop.text = string[9]
    }
